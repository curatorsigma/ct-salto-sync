...
async fn get_all_bookings(db: &Pool<Sqlite>) -> Result<Vec<Booking>, DBError> {
    Ok(sqlx::query_as!(
        NaiveBooking,
        "SELECT id, resource_id, start_time, end_time, description, created_by FROM bookings;"
    )
    .fetch_all(db)
    .await
...
pub async fn insert_booking(db: &Pool<Sqlite>, booking: &Booking) -> Result<(), DBError> {
    ...
    sqlx::query!(
        "INSERT INTO bookings (id, resource_id, start_time, end_time) VALUES \
        (?, ?, ?, ?);
        ",
        booking.id,
        booking.resource_id,
...
pub async fn delete_booking(db: &Pool<Sqlite>, id: i64) -> Result<(), DBError> {
    sqlx::query!(
...
pub async fn update_bookings<'a, I: Iterator<Item = &'a Booking>>(
    ...
    for b in bookings {
        ...
        info!("Updated Booking {}. Is now: {:?}", b.id, b)
    }
    Ok(())
}
